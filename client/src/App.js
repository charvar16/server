...
function App() {
  ...
  function newGame(){
    ...
    setRoomTest(socket.id)
    console.log(`(line 29 App.js debug) room.value: ${room.value}, roomTest: ${roomTest}`)
  ...
  const joinGame = (creator) =>{
    ...
    setRoomTest(creator)
    console.log(`(line 39 App.js debug) room.value: ${room.value}, roomTest: ${roomTest}`)
  ...
  const clickedSquare = (i) => {
    ...
    console.log(`(line 55 App.js debug) CLICKEDSQUARE - i: ${i}, id: ${id}, room.value: ${room.value}, roomTest: ${roomTest}`)
            ...
            <Route path="/" element={<OpenGames games={board} joinGame={joinGame}/>} />
              <Route path="/Game" element={
                <MoveContextProvider value={{clickedSquare, currentSquares, status}}>
                  <Game exitGame={exitGame} roomTest={roomTest}/>
                </MoveContextProvider>                
              } />
